...
class GroupRuleTest {
    ...
    fun testDefaultsConstructor() {
        val rule = GroupRule(GroupType.MANDATORY, g1, setOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE)
        ...
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo("mandatory group g1 contains [f1, f2, f3]")
    ...
    @Test
    fun testConstructor() {
        val rule = GroupRule(GroupType.OPTIONAL, g2, setOf(f1, f2, f3, f1, f2, f4), Visibility.PUBLIC, DEFAULT_MODULE)
        ...
        assertThat(rule.description).isEqualTo("")
        assertThat(rule.properties).isEmpty()
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo("optional group `g 2` contains [f1, f2, f3, `f#4`]")
    ...
    fun testFullConstructor() {
        val rule = GroupRule(
            GroupType.MANDATORY, g1, listOf(f1, f2, f3), Visibility.INTERNAL, DEFAULT_MODULE, "id string", "text text",
            properties
        )
        assertThat(rule.type).isEqualTo(GroupType.MANDATORY)
        assertThat(rule.group).isEqualTo(g1)
        ...
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo(
            "mandatory internal group g1 contains [f1, f2, f3] {" + System.lineSeparator() +
    ...
    @Test
    fun testEquals() {
        val rule = GroupRule(GroupType.MANDATORY, g1, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text", properties)
        val rule1 = GroupRule(GroupType.MANDATORY, g1, listOf(f1, f2, f3, f1, f2), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text", properties)
        val rule2 = GroupRule(GroupType.MANDATORY, g1, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string 2", "text text", properties)
        val rule3 = GroupRule(GroupType.MANDATORY, g1, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text 2", properties)
        val rule4 = GroupRule(
            GroupType.MANDATORY, g1, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text",
            mapOf(Pair("p1", EnumProperty("p1", EnumRange.list("text 1"))))
        )
        val rule5 = GroupRule(GroupType.OPTIONAL, g1, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text", properties)
        val rule6 = GroupRule(GroupType.MANDATORY, g2, listOf(f1, f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text", properties)
        val rule7 = GroupRule(GroupType.MANDATORY, g1, listOf(f2, f3), Visibility.PUBLIC, DEFAULT_MODULE, "id string", "text text", properties)
    ...
    @Test
    fun testFeatures() {
        val rule = GroupRule(GroupType.MANDATORY, g1, listOf(f1, f2, f3), module = DEFAULT_MODULE)
    ...
    fun testEvaluateMandatory() {
        ...
        val f2 = boolFt("f2")
        val f3 = boolFt("f3")
        val rule = GroupRule(GroupType.MANDATORY, g, setOf(f1, f2, f3), module = DEFAULT_MODULE)
    ...
    fun testEvaluateOptional() {
        ...
        val f2 = boolFt("f2")
        val f3 = boolFt("f3")
        val rule = GroupRule(GroupType.OPTIONAL, g, setOf(f1, f2, f3), module = DEFAULT_MODULE)
        val ass: FeatureAssignment = FeatureAssignment().assign(g, true).assign(f1, true).assign(f2, false).assign(f3, false)
        assertThat(rule.evaluate(ass)).isTrue
        ass.assign(f1, false)
        assertThat(rule.evaluate(ass)).isFalse
        ass.assign(f1, true)
    ...
    fun testRestrictMandatory() {
        ...
        assertThat(rule.restrict(ass1)).isEqualTo(ConstraintRule(exo(f1, f2, f3), module = DEFAULT_MODULE))

        // mandatory group false contains [f1, f2, f3] -> false
        val ass2: FeatureAssignment = FeatureAssignment().assign(g, false)
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(FALSE, module = DEFAULT_MODULE))
        ...
        assertThat(rule.restrict(ass3)).isEqualTo(ConstraintRule(g, module = DEFAULT_MODULE))

        // mandatory group g contains [true, true, f3] -> false
        val ass4: FeatureAssignment = FeatureAssignment().assign(f1, true).assign(f2, true)
        assertThat(rule.restrict(ass4)).isEqualTo(ConstraintRule(FALSE, module = DEFAULT_MODULE))
        ...
        assertThat(rule.restrict(ass5)).isEqualTo(ConstraintRule(FALSE, module = DEFAULT_MODULE))
        ...
        assertThat(rule.restrict(ass6)).isEqualTo(ConstraintRule(and(g, f3), module = DEFAULT_MODULE))

        // mandatory group g contains [true, f2, f3] -> g & -(f2 / f3) -> g & -f2 & -f3
        val ass7: FeatureAssignment = FeatureAssignment().assign(f1, true)
        assertThat(rule.restrict(ass7)).isEqualTo(ConstraintRule(and(g, not(f2), not(f3)), module = DEFAULT_MODULE))
        ...
        assertThat(rule.restrict(ass8)).isEqualTo(GroupRule(GroupType.MANDATORY, g, listOf(f2, f3), Visibility.PUBLIC, module = DEFAULT_MODULE))
    }
    ...
    fun testRestrictOptional() {
        ...
        val ass2: FeatureAssignment = FeatureAssignment().assign(g, false)
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(not(or(f1, f2, f3)), module = DEFAULT_MODULE))
        ...
        val ass5: FeatureAssignment = FeatureAssignment().assign(f1, false).assign(f2, false).assign(f3, false)
        assertThat(rule.restrict(ass5)).isEqualTo(ConstraintRule(not(g), module = DEFAULT_MODULE))

        // optional group g contains [false, false, f3] ->  g <=> f3
        val ass6: FeatureAssignment = FeatureAssignment().assign(f1, false).assign(f2, false)
        assertThat(rule.restrict(ass6)).isEqualTo(ConstraintRule(equiv(g, f3), module = DEFAULT_MODULE))
        ...
        val ass8: FeatureAssignment = FeatureAssignment().assign(f1, false)
        assertThat(rule.restrict(ass8)).isEqualTo(GroupRule(GroupType.OPTIONAL, g, setOf(f2, f3), module = DEFAULT_MODULE))
    }

    @Test
    fun testSyntacticSimplify() {
        val g = boolFt("g")
        val rule1 = GroupRule(GroupType.OPTIONAL, g, setOf(), module = DEFAULT_MODULE)
        val rule2 = GroupRule(GroupType.OPTIONAL, g, setOf(f1), module = DEFAULT_MODULE)
        val rule3 = GroupRule(GroupType.OPTIONAL, g, setOf(f1, f2, f3), module = DEFAULT_MODULE)
        val rule4 = GroupRule(GroupType.MANDATORY, g, setOf(), module = DEFAULT_MODULE)
        val rule5 = GroupRule(GroupType.MANDATORY, g, setOf(f1), module = DEFAULT_MODULE)
        val rule6 = GroupRule(GroupType.MANDATORY, g, setOf(f1, f2, f3), module = DEFAULT_MODULE)
        assertThat(rule1.syntacticSimplify()).isEqualTo(ConstraintRule(not(g), module = DEFAULT_MODULE))
        assertThat(rule2.syntacticSimplify()).isEqualTo(ConstraintRule(equiv(g, f1), module = DEFAULT_MODULE))
        assertThat(rule3.syntacticSimplify()).isEqualTo(rule3)
        assertThat(rule4.syntacticSimplify()).isEqualTo(ConstraintRule(FALSE, module = DEFAULT_MODULE))
        assertThat(rule5.syntacticSimplify()).isEqualTo(ConstraintRule(and(g, f1), module = DEFAULT_MODULE))
