...
class ExclusionRuleTest {
    ...
    fun testDefaultsConstructor() {
        val rule = ExclusionRule(feature, constraint, DEFAULT_MODULE)
        ...
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo("rule if f thenNot a & b")
    ...
    fun testFullConstructor() {
        val rule = ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        assertThat(rule.ifConstraint).isEqualTo(feature)
        assertThat(rule.thenNotConstraint).isEqualTo(constraint)
    ...
    fun testEquals() {
        val rule = ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule1 = ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule2 = ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string 2", "text text", properties)
        val rule3 = ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text 2", properties)
        val rule4 =
            ExclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", mapOf(Pair("p1", EnumProperty("p1", EnumRange.list("text 1")))))
        val rule5 = ExclusionRule(boolFt("f2"), constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule6 = ExclusionRule(feature, TRUE, DEFAULT_MODULE, "id string", "text text", properties)
    ...
    fun testFeatures() {
        ...
        val c2 = and(or(boolFt("b"), intLt(intFt("i"), 8), versionGt(versionFt("v"), 3)), intGt(intFt("x"), 3), or(boolFt("b"), enumEq(enumFt("desc"), "a")))
        val rule = ExclusionRule(c1, c2, DEFAULT_MODULE)
        val rule2 = ExclusionRule(
            enumIn(enumFt("f1"), "a", "b"),
            or(enumEq(enumFt("f1"), "c"), enumEq(enumFt("f1"), "a"), enumEq(enumFt("f2"), "c")), DEFAULT_MODULE
        )
    ...
    fun testEvaluate() {
        ...
        val rule = ExclusionRule(feature, and, DEFAULT_MODULE)
    ...
    fun testRestrict() {
        ...
        assertThat(rule.restrict(ass)).isEqualTo(ConstraintRule(not(a), DEFAULT_MODULE))
        ass.assign(b, false)
        ...
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(not(feature), DEFAULT_MODULE))
        // if f1 thenNot false -> ConstraintRule(true) -> -f1 / true -> true
        ass2.assign(a, false)
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        ...
        val rule2 = ExclusionRule(constraint2, constraint2, DEFAULT_MODULE)
        assertThat(rule2.restrict(ass3)).isEqualTo(ConstraintRule(not(boolFt("x")), DEFAULT_MODULE))

        // if f1 thenNot f2 -> ExclusionRule(f1r, f2r)
        val rule3 = ExclusionRule(and(boolFt("a"), boolFt("b")), and(boolFt("a"), boolFt("c")), DEFAULT_MODULE)
        assertThat(rule3.restrict(ass3)).isEqualTo(ExclusionRule(b, boolFt("c"), DEFAULT_MODULE))
    }

    @Test
    fun testSyntacticSimplify() {
        val constraint1 = and(boolFt("f"), intEq(intFt("i"), 2), TRUE)
        val rule1 = ExclusionRule(TRUE, constraint1, DEFAULT_MODULE)
        assertThat(rule1.syntacticSimplify()).isEqualTo(ConstraintRule(not(constraint1.syntacticSimplify()), DEFAULT_MODULE))
        val rule2 = ExclusionRule(and(feature, FALSE), constraint1, DEFAULT_MODULE)
        assertThat(rule2.syntacticSimplify()).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        val constraint3 = or(boolFt("b"), TRUE)
        val rule3 = ExclusionRule(feature, constraint3, DEFAULT_MODULE)
        assertThat(rule3.syntacticSimplify()).isEqualTo(ConstraintRule(not(feature), DEFAULT_MODULE))
        val constraint4 = and(boolFt("f"), not(TRUE))
        val rule4 = ExclusionRule(feature, constraint4, DEFAULT_MODULE)
        assertThat(rule4.syntacticSimplify()).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        val rule5 = ExclusionRule(constraint1, constraint1, DEFAULT_MODULE)
        assertThat(rule5.syntacticSimplify()).isEqualTo(ConstraintRule(not(constraint1.syntacticSimplify()), DEFAULT_MODULE))

        // if f1 thenNot f2 -> ExclusionRule(f1r, f2r)
        val rule6 = ExclusionRule(feature, constraint1, DEFAULT_MODULE)
        assertThat(rule6.syntacticSimplify()).isEqualTo(ExclusionRule(feature, constraint1.syntacticSimplify(), DEFAULT_MODULE))
    }
    ...
    fun testRenaming() {
        ...
        val rule = ExclusionRule(ifPart, thenNotPart, DEFAULT_MODULE)
