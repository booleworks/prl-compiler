...
class IfThenElseRuleTest {
    ...
    fun testDefaultsConstructor() {
        val rule = IfThenElseRule(feature, constraint1, constraint2, DEFAULT_MODULE)
        ...
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo("rule if f then a & b else -a / b")
    ...
    fun testFullConstructor() {
        val rule = IfThenElseRule(
            feature, constraint1, constraint2, DEFAULT_MODULE, "id string", "text text", properties
        )
        assertThat(rule.ifConstraint).isEqualTo(feature)
        assertThat(rule.thenConstraint).isEqualTo(constraint1)
    ...
    @Test
    fun testEquals() {
        val rule = IfThenElseRule(feature, constraint1, constraint2, DEFAULT_MODULE, "id string", "text text", properties)
        val rule1 = IfThenElseRule(feature, constraint1, constraint2, DEFAULT_MODULE, "id string", "text text", properties)
        val rule2 = IfThenElseRule(feature, constraint1, constraint2, DEFAULT_MODULE, "id string 2", "text text", properties)
        val rule3 = IfThenElseRule(feature, constraint1, constraint2, DEFAULT_MODULE, "id string", "text text 2", properties)
        val rule4 = IfThenElseRule(
            feature, constraint1, constraint2, DEFAULT_MODULE, "id string", "text text", mapOf(Pair("p1", EnumProperty("p1", EnumRange.list("text 1"))))
        )
        val rule5 = IfThenElseRule(boolFt("f2"), constraint1, constraint2, DEFAULT_MODULE, "id string", "text text", properties)
        val rule6 = IfThenElseRule(feature, TRUE, constraint2, DEFAULT_MODULE, "id string", "text text", properties)
        val rule7 = IfThenElseRule(feature, constraint1, TRUE, DEFAULT_MODULE, "id string", "text text", properties)
    ...
    fun testFeatures() {
        ...
        val rule = IfThenElseRule(c1, c2, c3, DEFAULT_MODULE)
        val rule2 = IfThenElseRule(
            enumIn(enumFt("f1"), "a", "b"),
            enumEq(enumFt("f1"), "c"),
            or(enumEq(enumFt("f1"), "a"), enumEq(enumFt("f2"), "c")), DEFAULT_MODULE
        )
    ...
    fun testEvaluate() {
        ...
        val c1 = and(boolFt("a"), enumEq(enumFt("name"), "desc"), or(boolFt("b"), intLt(intFt("i"), 8), versionLt(versionFt("v"), 8)))
        val c2 = and(intGt(intFt("x"), 3), or(boolFt("b"), enumEq(enumFt("desc"), "a")))
        val rule = IfThenElseRule(feature, c1, c2, DEFAULT_MODULE)
    ...
    fun testRestrict() {
        ...
        val rule = IfThenElseRule(constraint1, constraint2, constraint3, DEFAULT_MODULE)
        val ass1: FeatureAssignment = FeatureAssignment().assign(b, false).assign(c, false).assign(i, 7)
        assertThat(rule.restrict(ass1)).isEqualTo(ConstraintRule(a, DEFAULT_MODULE))
        val ass2: FeatureAssignment = FeatureAssignment().assign(a, true).assign(b, false)
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(intGe(intFt("i"), 7), DEFAULT_MODULE))
        val ass3: FeatureAssignment = FeatureAssignment().assign(a, false).assign(b, false)
        assertThat(rule.restrict(ass3)).isEqualTo(ConstraintRule(c, DEFAULT_MODULE))
        val ass4: FeatureAssignment = FeatureAssignment().assign(a, true).assign(i, 20)
        assertThat(rule.restrict(ass4)).isEqualTo(InclusionRule(not(constraint1.restrict(ass4)), constraint3.restrict(ass4), DEFAULT_MODULE))
        val ass5: FeatureAssignment = FeatureAssignment().assign(b, false).assign(i, 27)
        assertThat(rule.restrict(ass5)).isEqualTo(InclusionRule(not(a), constraint3.restrict(ass5), DEFAULT_MODULE))
        val ass6: FeatureAssignment = FeatureAssignment().assign(a, true).assign(i, 2)
        assertThat(rule.restrict(ass6)).isEqualTo(ConstraintRule(and(not(constraint1.restrict(ass6)), constraint3.restrict(ass6)), DEFAULT_MODULE))
        val ass7: FeatureAssignment = FeatureAssignment().assign(c, true)
        assertThat(rule.restrict(ass7)).isEqualTo(InclusionRule(constraint1.restrict(ass7), constraint2.restrict(ass7), DEFAULT_MODULE))
        val ass8: FeatureAssignment = FeatureAssignment().assign(a, false).assign(c, false)
        assertThat(rule.restrict(ass8)).isEqualTo(
            ConstraintRule(
                and(constraint1.restrict(ass8), constraint2.restrict(ass8)).syntacticSimplify(),
                DEFAULT_MODULE
            )
        )
        ...
        assertThat(rule.restrict(ass9)).isEqualTo(
            ConstraintRule(
                and(constraint1.restrict(ass9), constraint2.restrict(ass9)).syntacticSimplify(),
                DEFAULT_MODULE
            )
        )
        val ass10: FeatureAssignment = FeatureAssignment().assign(b, false)
        assertThat(rule.restrict(ass10)).isEqualTo(IfThenElseRule(constraint1.restrict(ass10), constraint2, constraint3, DEFAULT_MODULE))
    }
    ...
    fun testSyntacticSimplify() {
        ...
        val c = boolFt("c")
        val s = enumFt("s")
        assertThat(IfThenElseRule(TRUE, b, c, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(b, DEFAULT_MODULE))
        assertThat(IfThenElseRule(and(a, FALSE), b, c, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(c, DEFAULT_MODULE))
        assertThat(IfThenElseRule(not(not(a)), a, a, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(a, DEFAULT_MODULE))
        assertThat(IfThenElseRule(a, impl(a, TRUE), c, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(InclusionRule(not(a), c, DEFAULT_MODULE))
        assertThat(IfThenElseRule(a, b, TRUE, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(InclusionRule(a, b, DEFAULT_MODULE))
        assertThat(IfThenElseRule(a, equiv(not(a), FALSE), c, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(InclusionRule(not(a), c, DEFAULT_MODULE))
        assertThat(IfThenElseRule(a, and(not(b), FALSE), c, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(and(not(a), c), DEFAULT_MODULE))
        assertThat(IfThenElseRule(a, b, FALSE, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(and(a, b), DEFAULT_MODULE))
        assertThat(IfThenElseRule(and(a, a), b, a, DEFAULT_MODULE).syntacticSimplify()).isEqualTo(ConstraintRule(and(a, b), DEFAULT_MODULE))
        assertThat(IfThenElseRule(and(not(a), TRUE), and(b, b), enumEq(s, "name"), DEFAULT_MODULE).syntacticSimplify()).isEqualTo(
            IfThenElseRule(not(a), b, enumEq(s, "name"), DEFAULT_MODULE)
        )
    }
    ...
    fun testRenaming() {
        ...
        val rule = IfThenElseRule(ifPart, thenPart, elsePart, DEFAULT_MODULE)
