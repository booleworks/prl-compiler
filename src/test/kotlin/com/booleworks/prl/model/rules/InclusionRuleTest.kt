...
class InclusionRuleTest {
    ...
    fun testDefaultsConstructor() {
        val rule = InclusionRule(feature, constraint, DEFAULT_MODULE)
        ...
        assertThat(rule.toString(DEFAULT_MODULE)).isEqualTo("rule if f then a & b")
    ...
    @Test
    fun testBuilder() {
        val rule = InclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        assertThat(rule.ifConstraint).isEqualTo(feature)
        assertThat(rule.thenConstraint).isEqualTo(constraint)
    ...
    fun testEquals() {
        val rule = InclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule1 = InclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule2 =
            InclusionRule(feature, constraint, DEFAULT_MODULE, "id string 2", "text text", properties)
        val rule3 =
            InclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text 2", properties)
        val rule4 =
            InclusionRule(feature, constraint, DEFAULT_MODULE, "id string", "text text", mapOf(Pair("p1", EnumProperty("p1", EnumRange.list("text 1")))))
        val rule5 = InclusionRule(boolFt("f2"), constraint, DEFAULT_MODULE, "id string", "text text", properties)
        val rule6 = InclusionRule(feature, TRUE, DEFAULT_MODULE, "id string", "text text", properties)
    ...
    fun testFeatures() {
        ...
        val rule = InclusionRule(c1, c2, DEFAULT_MODULE)
    ...
    fun testEvaluate() {
        ...
        val rule = InclusionRule(feature, and, DEFAULT_MODULE)
    ...
    fun testRestrict() {
        ...
        assertThat(rule.restrict(ass1)).isEqualTo(ConstraintRule(b, DEFAULT_MODULE))
        assertThat(rule.restrict(ass2)).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        assertThat(rule.restrict(ass3)).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        assertThat(rule.restrict(ass4)).isEqualTo(ConstraintRule(not(feature), DEFAULT_MODULE))
        assertThat(rule.restrict(ass5)).isEqualTo(InclusionRule(feature, b, DEFAULT_MODULE))
        val rule2 = InclusionRule(feature, feature, DEFAULT_MODULE)
        assertThat(rule2.restrict(ass5)).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
    }

    @Test
    fun testSyntacticSimplify() {
        val a = boolFt("a")
        val rule1 = InclusionRule(TRUE, constraint, DEFAULT_MODULE)
        val rule2 = InclusionRule(FALSE, constraint, DEFAULT_MODULE)
        val rule3 = InclusionRule(feature, or(a, TRUE), DEFAULT_MODULE)
        val rule4 = InclusionRule(feature, and(intGt(intFt("i"), 2), FALSE), DEFAULT_MODULE)
        val constraint5 = enumEq(enumFt("name"), "text")
        val rule5 = InclusionRule(constraint5, constraint5, DEFAULT_MODULE)
        val rule6 = InclusionRule(feature, and(a, TRUE), DEFAULT_MODULE)
        assertThat(rule1.syntacticSimplify()).isEqualTo(ConstraintRule(constraint, DEFAULT_MODULE))
        assertThat(rule2.syntacticSimplify()).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        assertThat(rule3.syntacticSimplify()).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        assertThat(rule4.syntacticSimplify()).isEqualTo(ConstraintRule(not(feature), DEFAULT_MODULE))
        assertThat(rule5.syntacticSimplify()).isEqualTo(ConstraintRule(TRUE, DEFAULT_MODULE))
        assertThat(rule6.syntacticSimplify()).isEqualTo(InclusionRule(feature, a, DEFAULT_MODULE))
    }
    ...
    fun testRenaming() {
        ...
        val rule = InclusionRule(ifPart, thenPart, DEFAULT_MODULE)
