...
class ConstraintCompilerTest {
    ...
    fun testCompileAmo() {
        ...
        assertThat(cc.compileConstraint(PrlAmo(PrlFeature("b1"), PrlFeature("b2"), PrlFeature("b3")), featureMap)).isEqualTo(amo(b1, b2, b3))
    ...
    fun testCompileExo() {
        ...
        assertThat(cc.compileConstraint(PrlExo(PrlFeature("b1"), PrlFeature("b2"), PrlFeature("b3")), featureMap)).isEqualTo(exo(b1, b2, b3))
    ...
    fun testImplication() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("-(b1&b2) => true"), featureMap)).isEqualTo(impl(not(and(b1, b2)), TRUE))
    ...
    fun testEquivalence() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("-(b1&b2) <=> true"), featureMap)).isEqualTo(equiv(not(and(b1, b2)), TRUE))
        assertThatThrownBy { cc.compileConstraint(parseConstraint("b1 <=> x"), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Unknown feature: 'x'")
        assertThatThrownBy { cc.compileConstraint(parseConstraint("x <=> b2"), featureMap) }
    ...
    fun testAnd() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("b1 & (b2/b3) & -b2"), featureMap)).isEqualTo(and(b1, or(b2, b3), not(b2)))
    ...
    fun testOr() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("b1 / b2 & b3 / -b2"), featureMap)).isEqualTo(or(b1, and(b2, b3), not(b2)))
        assertThatThrownBy { cc.compileConstraint(parseConstraint("true / x"), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Unknown feature: 'x'")
    }
    ...
    fun testNot() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("-(b1 / b2 & b3 / -b2)"), featureMap)).isEqualTo(not(or(b1, and(b2, b3), not(b2))))
    ...
    fun testEnumInPredicate() {
        assertThat(cc.compileConstraint(parseConstraint("""[e1 in ["a"]]"""), featureMap)).isEqualTo(enumIn(e1, listOf("a")))
        assertThat(cc.compileConstraint(parseConstraint("""[e1 in ["a", "b", "c"]]"""), featureMap)).isEqualTo(enumIn(e1, listOf("a", "b", "c")))
    ...
    @Test
    fun testIntMul() {
        assertThat(cc.compileIntTerm(PrlIntMulFunction(PrlFeature("i1"), PrlIntValue(3)), featureMap)).isEqualTo(intMul(3, i1))
        assertThat(cc.compileIntTerm(PrlIntMulFunction(PrlIntValue(-3), PrlFeature("i1")), featureMap)).isEqualTo(intMul(-3, i1))
        ...
        assertThatThrownBy { cc.compileIntTerm(PrlIntMulFunction(PrlIntMulFunction(PrlIntValue(3), PrlFeature("i")), PrlIntValue(3)), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Integer multiplication is only allowed between a fixed coefficient and an integer feature")
    ...
    fun testIntSum() {
        ...
        assertThat(cc.compileIntTerm(PrlIntAddFunction(PrlIntValue(13), mul1, PrlIntAddFunction(mul2, mul3)), featureMap))
            .isEqualTo(intSum(13, intMul(3, i1), intMul(2, i2), intMul(-4, i3)))
        ...
        assertThatThrownBy { cc.compileIntTerm(PrlIntAddFunction(PrlIntValue(13), mul1, PrlIntMulFunction(PrlIntValue(3), PrlFeature("x"))), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Unknown feature: 'x'")
        assertThatThrownBy { cc.compileIntTerm(PrlIntAddFunction(PrlIntValue(13), mul1, PrlIntMulFunction(PrlIntValue(3), PrlFeature("b1"))), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Boolean feature 'b1' is used as int feature")
    }
    ...
    fun testIntInPredicate() {
        assertThat(cc.compileConstraint(parseConstraint("[i1 in [-1 - 7]]"), featureMap)).isEqualTo(intIn(i1, IntRange.interval(-1, 7)))
        assertThat(cc.compileConstraint(parseConstraint("[i2*3 in [-1 - 7]]"), featureMap)).isEqualTo(intIn(intMul(3, i2), IntRange.interval(-1, 7)))
    ...
    fun testEnumComparisonPredicate() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("""["text" != e2]"""), featureMap)).isEqualTo(enumNe(e2, "text"))
    ...
    fun testIntComparisonPredicate() {
        ...
        assertThat(cc.compileConstraint(parseConstraint("[7 = 7]"), featureMap)).isEqualTo(intEq(7.toIntValue(), 7.toIntValue()))
    ...
    fun testVersionPredicate() {
        ...
        assertThat(cc.compileConstraint(PrlComparisonPredicate(EQ, PrlIntValue(4), PrlFeature("v")), featureMap)).isEqualTo(versionEq(v, 4))
        assertThat(cc.compileConstraint(PrlComparisonPredicate(EQ, PrlIntValue(4), PrlFeature("v")), featureMap)).isEqualTo(versionEq(v, 4))
        assertThatThrownBy { cc.compileConstraint(PrlComparisonPredicate(EQ, PrlIntValue(-4), PrlFeature("v")), featureMap) }
            .isInstanceOf(CoCoException::class.java)
            .hasMessage("Versions must be > 0")
        assertThatThrownBy { cc.compileConstraint(PrlComparisonPredicate(EQ, PrlFeature("v"), PrlFeature("v")), featureMap) }
            .isInstanceOf(CoCoException::class.java)
    ...
    fun testLargeExample() {
        val formula = """b1 => ([e1 in ["a", "b", "c"]] / -b2) & -(v[>=7] <=> [3*i3 + 47 > 8*i1 + -2*i2] / [-2*i2 + 4 in [-1, 1, 10]])"""
        assertThat(cc.compileConstraint(parseConstraint(formula), featureMap).toString(DEFAULT_MODULE)).isEqualTo(formula)
    }
}
