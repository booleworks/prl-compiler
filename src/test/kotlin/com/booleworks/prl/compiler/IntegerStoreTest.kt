...
class IntegerStoreTest {
    ...
    fun testInPredicate() {
        ...
        assertThat(store.usedValues[x]).isEqualTo(UsageInfo(values = sortedSetOf(IntRange.list(1, 2, 3))))
        assertThat(store.usedValues[y]).isEqualTo(UsageInfo(values = sortedSetOf(IntRange.interval(-3, 3))))
        ...
        assertThat(store.usedValues[z]).isEqualTo(UsageInfo(usedInArEx = true))
    }
    ...
    fun testCompPredicate() {
        ...
        assertThat(store.usedValues[x]).isEqualTo(UsageInfo(values = sortedSetOf(IntRange.list(7))))

        store.addUsage(intLe(intVal(3), x))
        assertThat(store.usedValues).hasSize(1)
        assertThat(store.usedValues[x]).isEqualTo(UsageInfo(values = sortedSetOf(IntRange.list(7), IntRange.list(3))))
        ...
        assertThat(store.usedValues).hasSize(3)
        assertThat(store.usedValues[y]).isEqualTo(UsageInfo(usedInArEx = true))
        assertThat(store.usedValues[z]).isEqualTo(UsageInfo(usedInArEx = true))

        store.addUsage(intGt(intSum(intMul(1, p)), q))
        assertThat(store.usedValues).hasSize(5)
        assertThat(store.usedValues[p]).isEqualTo(UsageInfo(otherFeatures = sortedSetOf(q)))
        assertThat(store.usedValues[q]).isEqualTo(UsageInfo(otherFeatures = sortedSetOf(p)))
    }
}
