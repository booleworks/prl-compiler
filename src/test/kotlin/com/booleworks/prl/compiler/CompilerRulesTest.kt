...
class CompilerRulesTest {
    ...
    fun testConstraintRule() {
        ...
        val rule = PrlCompiler().compileRule(parseRule("rule (b1 & b2) <=> [i1 < 7]"), module, featureMap, intStore)
        assertThat(rule).isEqualTo(ConstraintRule(equiv(and(b1, b2), intLt(i1, 7)), module))

        PrlCompiler().let {
            assertThat(it.compileRule(parseRule("rule (b1 & b2) <=> [e1 < 7]"), module, featureMap, intStore)).isNull()
    ...
    fun testDefinitionRule() {
        val intStore = IntegerStore()
        val rule =
            PrlCompiler().compileRule(parseRule("""rule b1 is b2 => [e1 = "text"]"""), module, featureMap, intStore)
        assertThat(rule).isEqualTo(DefinitionRule(b1, impl(b2, enumEq(e1, "text")), module))

        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule v is b2 => [e1 = "text"]"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule i1 is b2 => [e1 = "text"]"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule b1 is x => [e1 = "text"]"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testExclusionRule() {
        val intStore = IntegerStore()
        val rule =
            PrlCompiler().compileRule(parseRule("""rule if b1 thenNot [i1 in [1-10]]"""), module, featureMap, intStore)
        assertThat(rule).isEqualTo(ExclusionRule(b1, intIn(i1, IntRange.interval(1, 10)), module))

        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if x thenNot [i1 in [1-10]]"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if b1 thenNot [i5 in [1-10]]"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testForbiddenFeatureRule() {
        val intStore = IntegerStore()
        val rule1 = PrlCompiler().compileRule(parseRule("""rule forbidden feature b1"""), module, featureMap, intStore)
        assertThat(rule1).isEqualTo(ForbiddenFeatureRule(b1, module))
        val rule2 =
            PrlCompiler().compileRule(parseRule("""rule forbidden feature v[=2]"""), module, featureMap, intStore)
        assertThat(rule2).isEqualTo(ForbiddenFeatureRule(v, 2, module))
        val rule3 =
            PrlCompiler().compileRule(parseRule("""rule forbidden feature e1 = "text""""), module, featureMap, intStore)
        assertThat(rule3).isEqualTo(ForbiddenFeatureRule(e1, "text", module))
        val rule4 =
            PrlCompiler().compileRule(parseRule("""rule forbidden feature i1 = 8"""), module, featureMap, intStore)
        assertThat(rule4).isEqualTo(ForbiddenFeatureRule(i1, 8, module))

        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule forbidden feature b1 = 2"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(it.compileRule(parseRule("""rule forbidden feature v"""), module, featureMap, intStore)).isNull()
            assertThat(it.errors()).containsExactly("Cannot assign a versioned boolean feature to anything else than an int version")
        }
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule forbidden feature e1 = 7"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule forbidden feature e1"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule forbidden feature i1"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testGroupRule() {
        val intStore = IntegerStore()
        val rule1 = PrlCompiler().compileRule(
            parseRule("""optional group b1 contains [b2, b3]"""),
            module,
            featureMap,
            intStore
        )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""optional group v contains [b2, b3]"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testIfThenElseRule() {
        val intStore = IntegerStore()
        val rule1 = PrlCompiler().compileRule(
            parseRule("""rule if b1 then b2 else [i1 <= -10]"""),
            module,
            featureMap,
            intStore
        )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if y then b2 else [i1 <= -10]"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if b1 then y else [i1 <= -10]"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testInclusionRule() {
        val intStore = IntegerStore()
        val rule =
            PrlCompiler().compileRule(parseRule("""rule if b1 then [i1 in [1-10]]"""), module, featureMap, intStore)
        assertThat(rule).isEqualTo(InclusionRule(b1, intIn(i1, IntRange.interval(1, 10)), module))

        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if x then [i1 in [1-10]]"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule if b1 then [i5 in [1-10]]"""),
                    module,
                    featureMap,
                    intStore
                )
    ...
    fun testMandatoryFeatureRule() {
        val intStore = IntegerStore()
        val rule1 = PrlCompiler().compileRule(parseRule("""rule mandatory feature b1"""), module, featureMap, intStore)
        assertThat(rule1).isEqualTo(MandatoryFeatureRule(b1, module))
        val rule2 =
            PrlCompiler().compileRule(parseRule("""rule mandatory feature v[=2]"""), module, featureMap, intStore)
        assertThat(rule2).isEqualTo(MandatoryFeatureRule(v, 2, module))
        val rule3 =
            PrlCompiler().compileRule(parseRule("""rule mandatory feature e1 = "text""""), module, featureMap, intStore)
        assertThat(rule3).isEqualTo(MandatoryFeatureRule(e1, "text", module))
        val rule4 =
            PrlCompiler().compileRule(parseRule("""rule mandatory feature i1 = 8"""), module, featureMap, intStore)
        assertThat(rule4).isEqualTo(MandatoryFeatureRule(i1, 8, module))

        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule mandatory feature b1 = 2"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(it.compileRule(parseRule("""rule mandatory feature v"""), module, featureMap, intStore)).isNull()
            assertThat(it.errors()).containsExactly("Cannot assign a versioned boolean feature to anything else than an int version")
        }
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule mandatory feature e1 = 7"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule mandatory feature e1"""),
                    module,
                    featureMap,
                    intStore
                )
        ...
        PrlCompiler().let {
            assertThat(
                it.compileRule(
                    parseRule("""rule mandatory feature i1"""),
                    module,
                    featureMap,
                    intStore
                )
