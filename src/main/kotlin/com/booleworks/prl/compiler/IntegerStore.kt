...
data class IntegerStore internal constructor(
    val usedValues: MutableMap<IntFeature, UsageInfo> = mutableMapOf()
) {
    ...
    private fun addComparisonPredicate(predicate: IntComparisonPredicate) {
        ...
        if (left is IntMul || left is IntSum || right is IntMul || right is IntSum) {
            ...
            usedValues.computeIfAbsent(left) { UsageInfo() }.otherFeatures.add(right)
            usedValues.computeIfAbsent(right) { UsageInfo() }.otherFeatures.add(left)
        } else {
            val feature = if (left is IntFeature) left else right as IntFeature
            val value = if (left is IntValue) left else right as IntValue
            usedValues.computeIfAbsent(feature) { UsageInfo() }.values.add(IntRange.list(value.value))
        }
    }

    private fun addInPredicate(predicate: IntInPredicate) {
        when (predicate.term) {
            is IntFeature -> usedValues.computeIfAbsent(predicate.term) { UsageInfo() }.values.add(predicate.range)
    ...
    private fun addArithmeticExpression(term: IntTerm) {
        term.features().forEach { feature ->
            usedValues.computeIfAbsent(feature) { UsageInfo() }.usedInArEx = true
        }
    }
}
